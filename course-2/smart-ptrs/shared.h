...
class ControlBlockPointer : public ControlBlockBase {
   public:
    ControlBlockPointer(U* ptr) : ptr_(ptr) { std::cout << "[memory_leak] ControlBlockPointer(U* ptr)\n"; }
    U* ptr_;

    virtual ~ControlBlockPointer() {
        std::cout << "[memory_leak] ~ControlBlockPointer\n";
        delete ptr_;
    }
};
